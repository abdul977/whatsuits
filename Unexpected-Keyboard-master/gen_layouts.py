...
def read_layout(fname):
    ...
    root = XML.parse(fname, parser=parser).getroot()
...
def generate_arrays(out, layouts):
    ...
    XML.indent(root)
    # Work around Python's XML writing quirks by writing the declaration manually
    out.write('<?xml version="1.0" encoding="utf-8"?>\n')
    XML.ElementTree(element=root).write(out, encoding="unicode", xml_declaration=False)
    out.write('\n')  # Add final newline

layouts = sort_layouts(read_layouts(glob.glob("srcs/layouts/*.xml")))
with open("res/values/layouts.xml", "w", encoding="utf-8") as out:
